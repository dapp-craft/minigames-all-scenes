...
const generateInitialEntity = async () => {
  ...
  syncEntity(
    ...
    TOADS_SYNC_ID + toadsGameConfig.ToadsAmount + 10 + 1
  )
  ...
  syncEntity(
    counter,
    [Transform.componentId, TextShape.componentId],
    TOADS_SYNC_ID + toadsGameConfig.ToadsAmount + 10 + 5
  )
  ...
  for (let i = 0; i < toadsGameConfig.ToadsAmount; i++) {
    if (
      Transform.getOrNull(toadsGameState.availableEntity[i]) == null ||
      GltfContainer.getOrNull(toadsGameState.availableEntity[i]) == null
    ) {
      Transform.create(toadsGameState.availableEntity[i], {
        position: { ...data.get(`obj_frog_hidden_${i + 1}`)!.position, y: toadsGameState.toadInitialHeight },
        parent: sceneParentEntity
      })
      GltfContainer.createOrReplace(toadsGameState.availableEntity[i], {
        src: frog01.src,
        visibleMeshesCollisionMask: ColliderLayer.CL_CUSTOM5
      })
    }
  }
}
